...
# --- CONFIGURATION PAGE (Hamburger Menu par defaut) ---
st.set_page_config(
...
<style>
    ...
    html, body, [class*="css"] { font-family: 'Inter', sans-serif; background-color: #0B0E14; color: #E1E1E1; }
...
def fetch_realtime_data():
    ...
    if psycopg2:
        try:
            ...
            last_error = f"SQL Error: {str(e)} | Switching to CSV..."
...
with st.sidebar:
    ...
    # Affichage conditionnel du diagnostic
    if sql_error:
    ...
    st.markdown("---")
    st.markdown("#### Controle Architecture")
    arch = st.radio("Mode :", ["Mode FOG", "Mode EDGE"])
    if st.button("Appliquer Configuration", use_container_width=True):
        try: 
            publish.single("irrigation/mode", "FOG" if "FOG" in arch else "EDGE", hostname="127.0.0.1") 
...
if not df.empty:
    st.markdown("### Smart Irrigation - Raspberry Pi Fog Computing Dashboard")
    
    # --- MASTER GRAPH : HUMIDITY + PUMP FUSION (Double Axe Y) ---
    from plotly.subplots import make_subplots
    
    # Preparation des donnees
    df_pivot_h = df.pivot_table(index='ts', columns='node', values='h').ffill().fillna(50)
    # On prend l'etat de la pompe max entre les noeuds (car une seule pompe physique)
    df_pump_single = df.groupby('ts')['pump'].max().ffill().fillna(0)
    
    f_combined = make_subplots(specs=[[{"secondary_y": True}]])
    
    # 1. Courbes Humidite (Axe Primaire)
    for node in df_pivot_h.columns:
        color = "#00FF7F" if 'node1' in str(node) else "#FFBB33"
        f_combined.add_trace(
            go.Scatter(x=df_pivot_h.index, y=df_pivot_h[node], name=f"Hums {node}", 
                      line=dict(color=color, width=2)),
            secondary_y=False,
        )
    
    # 2. Zone Pompage UNIQUE (Axe Secondaire)
    f_combined.add_trace(
        go.Scatter(x=df_pump_single.index, y=df_pump_single, name="Etat Pompe", 
                  fill='tozeroy', line=dict(color='rgba(0,0,0,0)'), mode='none',
                  fillcolor="rgba(75, 155, 255, 0.25)"),
        secondary_y=True,
    )

    f_combined.update_layout(
        title="Supervision IA : Humidite (N1/N2) & Etat Pompe Unique",
        template="plotly_dark", height=400,
        margin=dict(l=0,r=0,b=0,t=40), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
    )
    f_combined.update_yaxes(title_text="Humidite (%)", range=[0, 105], secondary_y=False)
    ...
    # --- SECTION DECISION & ACTIVITE (Alignement Horizontal 4 colonnes) ---
    ...
    # Pre-calculs Node 1 & 2
    d1 = df[df['node'] == 'node1'] ; d2 = df[df['node'] == 'node2']
    h1 = d1['h'].iloc[-1] if not d1.empty else 0 ; h2 = d2['h'].iloc[-1] if not d2.empty else 0
    p1 = 1 if not d1.empty and d1['pump'].iloc[-1] else 0
    p2 = 1 if not d2.empty and d2['pump'].iloc[-1] else 0

    with c_g1:
        st.markdown('<div style="font-size:0.6rem; color:#808495; text-align:center;">HUMIDITE NODE 1</div>', unsafe_allow_html=True)
        f1 = go.Figure(go.Indicator(
            mode = "gauge+number", value = h1,
    ...
    with c_g2:
        st.markdown('<div style="font-size:0.6rem; color:#808495; text-align:center;">HUMIDITE NODE 2</div>', unsafe_allow_html=True)
        f2 = go.Figure(go.Indicator(
            mode = "gauge+number", value = h2,
    ...
    with c_stat:
        # Bannieres Empilees (Petite taille pour tenir verticalement)
    ...
    with c_count:
        ...
        st.markdown('<div style="font-size:0.6rem; color:#808495; text-align:center; margin-top:5px;">DECLENCHEMENTS POMPES (TOTAL)</div>', unsafe_allow_html=True)

    st.write("---")
    
    # --- PERFORMANCE SECTION (Alignement sur une seule ligne) ---
    st.markdown('<div class="section-header">⚡ Performance Edge vs Fog — Latence & R\u00e9seau</div>', unsafe_allow_html=True)
    cp1, cp2, cp3 = st.columns([10, 6, 6]) # R\u00e9partition optimale
    
    with cp1:
        f_lat = px.line(df, x='ts', y='decision_latency_ms', title="Latence de D\u00e9cision IA", height=280, template="plotly_dark")
        f_lat.update_layout(margin=dict(l=0,r=0,b=0,t=30), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    ...
    with cp3:
        ...
        # Jauge Batterie en demi-cercle (Style Grafana)
        fb = go.Figure(go.Indicator(
            mode = "gauge+number", value = batt_v,
    ...
    # ✅ FIXE : Au lieu d'afficher "Recherche de signal" indéfiniment,
